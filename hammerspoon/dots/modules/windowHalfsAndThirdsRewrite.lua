...
--- Variable
--- If `true`, set [setFrameCorrectness](http://www.hammerspoon.org/docs/hs.window.html#setFrameCorrectness) for some resizing operations which fail when the window extends beyonds screen boundaries.
--- This may cause some jerkiness in the resizing, so experiment and determine if you need it. Defaults to `false`
...
--
-- `window_state_names` are states windows can be in (so since `third_left` implies
-- a relative move there is no `third_left` `window_state_name`, only a `third_left` `action`)
