...
get_dotfiles_path() {
    ...
    local shells="$(dirname "$zsh")"
    if [[ "$(basename "$shells")" != "shells" ]]; then
        return 1
    fi

    local dotfiles="$(dirname "$shells")"
...
recursively_source_aliases "$DOTFILES/shells/common/aliases"
recursively_source_aliases "$DOTFILES/shells/common/functions"

# zsh-specific functions (completions, autoload bodies)
FPATH="$ZFUNCS:$FPATH"
