#!/bin/bash
#
# Finds the subdirectory in dots that matches the current machine's $HOSTNAME; if not found,
# prompts the user which machine's config to use, with an option to copy it and create a new
# config for the current machine. Symlinks the matching directory to ~/.config/waybar.
#
# Note that some machines contain multiple configs for different situations (e.g. rewgs-x1c11's
...
# Reads from the global _WAYBAR_CONFIGS array.
_waybar_tab_complete() {
    local -a matches
    mapfile -t matches < <(compgen -W "${_WAYBAR_CONFIGS[*]}" -- "${READLINE_LINE}")
...
main() {
    ...
    if [[ ! -d "${host_dir}" ]]; then
        ...
        _WAYBAR_CONFIGS=()
        for dir in "${dots_dir}"/*/; do
            _WAYBAR_CONFIGS+=("$(basename "${dir}")")
        done
        bind -x '"\t":_waybar_tab_complete' 2>/dev/null

        local chosen
        read -re -p "Which config to use as a base? " chosen

        bind '"\t":complete' 2>/dev/null
        unset _WAYBAR_CONFIGS
